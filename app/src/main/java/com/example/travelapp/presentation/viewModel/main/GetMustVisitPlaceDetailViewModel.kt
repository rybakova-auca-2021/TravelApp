...
class GetMustVisitPlaceDetailViewModel(private val repository: MainRepository) : ViewModel() {
    private val _result = MutableLiveData<Result<PlaceModel>>()
    val result: LiveData<Result<PlaceModel>>
        get() = _result

    fun getMustVisitDetailPlaces(id: Int) {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                val response = repository.getMustVisitPlaceDetail(id)
                response.enqueue(object : Callback<PlaceModel> {
                    override fun onResponse(call: Call<PlaceModel>, response: Response<PlaceModel>) {
                        if (response.isSuccessful) {
                            ...
                            _result.postValue(Result.failure(Throwable("Failed to get the detail of must visit places")))
                        }
                    }
